...
router.route("/references/count/counterparty")
    .get(async (req, res) => {
        ReferenceHandler.CounterpartyView.get_counterparty_count(req, res);
    });

router.route("/references/search/counterparty")
    .get(async (req, res) => {
        ReferenceHandler.CounterpartyView.get_counterparties_names(req, res);
    });
