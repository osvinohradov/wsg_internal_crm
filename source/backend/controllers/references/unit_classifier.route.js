...
router.route('/reference/unit_classifiers')
    // Отримуємо n єлементів за БД
    .get(async (req, res) => {
        References.ServiceTypeController.get_service_types(req, res);
    });

router.route('/reference/unit_classifiers/names')
    // Отримати всі існуючі рахунки
    .get(async (req, res) => {
        References.UnitClassifierController.get_unit_classifiers_names(req, res);
    });

router.route('/reference/unit_classifier')
    // Створюэмо новий елемент в таблиці
    .post(async (req, res) => {
        //References.ServiceTypeController.create_service_type(req, res);
    })

router.route('/reference/unit_classifier/:id')
...
router.route('/reference/count/unit_classifier')
