...
router.route('/reference/checking_accounts')
    // Отримуємо n єлементів за БД
    .get(async (req, res) => {
        References.ServiceTypeController.get_service_types(req, res);
    });

router.route('/reference/checking_accounts/names')
    // Отримати всі існуючі рахунки
    .get(async (req, res) => {
        References.CheckingAccountController.get_checking_accounts_names(req, res);
    });

router.route('/reference/checking_account')
    // Створюэмо новий елемент в таблиці
    .post(async (req, res) => {
        //References.ServiceTypeController.create_service_type(req, res);
    })

router.route('/reference/checking_account/:id')
...
router.route('/reference/count/checking_account')
