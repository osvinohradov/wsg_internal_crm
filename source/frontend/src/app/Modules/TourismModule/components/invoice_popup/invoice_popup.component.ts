...
export class TourismInvoiceDialogComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<TourismInvoiceDialogComponent>,
              @Inject(MAT_DIALOG_DATA) public tourism_invoice: TourismInvoiceDetail,
  ...
  setClientValue(event, value){
    this.tourism_invoice.client_id = value;
  }
  setGroupInvoiceValue(event, value){
    this.tourism_invoice.group_invoice_id = value;
  }
  setOfferCurrencyValue(event, value){
    this.tourism_invoice.offer_currency_id = value;
  }
  setTotalCurrencyValue(event, value){
    this.tourism_invoice.total_currency_id = value;
  }
  setProviderValue(event, value){
    this.tourism_invoice.provider_id = value;
  }
  setTaxesPaymentValue(event, value){
    this.tourism_invoice.taxes_payment_id = value;
  }
  setCuratorValue(event, value){
    this.tourism_invoice.curator_id = value;
  }
  setCurrencyExchangeValue(event, value){
    this.tourism_invoice.currency_exchange_id = value;
  }
  setServiceTypeValue(event, value){
    this.tourism_invoice.service_type_id = value;
  }
  setCheckingAccountValue(event, value){
    this.tourism_invoice.checking_account_id = value;
  }
  setResponsibleAgentValue(event, value){
    this.tourism_invoice.responsible_agent_id = value;
  }
  setAgentValue(event, value){
    this.tourism_invoice.agent_id = value;
  }
  setOrganizationValue(event, value){
    this.tourism_invoice.organization_id = value;
  }
  setDepartureStationValue(event, value){
    this.tourism_invoice.detail_info.departure_station_id = value;
  }
  setArrivalStationValue(event, value){
    this.tourism_invoice.detail_info.arrival_station_id = value;
  }
  setIndividualCounterpartyValue(event, value){
    this.tourism_invoice.detail_info.surname_id = value;
  }
  setSupplierCostCurrencyValue(event, value){
    this.tourism_invoice.detail_info.supplier_cost.currency_id = value;
  }
  ...
  ngOnInit() {
    ...
    console.log(this.tourism_invoice);
    this.clientFromControl.setValue(this.tourism_invoice.client_id.name);
    this.groupInvoiceFromControl.setValue(this.tourism_invoice.group_invoice_id.group_name);
    this.offerCurrencyFromControl.setValue(this.tourism_invoice.offer_currency_id.name);
    this.totalCurrencyFromControl.setValue(this.tourism_invoice.total_currency_id.name);
    this.providerFromControl.setValue(this.tourism_invoice.provider_id.name);
    this.providerFromControl.setValue(this.tourism_invoice.provider_id.name);
    this.taxesPaymentFromControl.setValue(this.tourism_invoice.taxes_payment_id.name);
    this.curatorFromControl.setValue(this.tourism_invoice.curator_id.name);
    this.currencyExchangeFromControl.setValue(this.tourism_invoice.currency_exchange_id.name);
    this.serviceTypeFromControl.setValue(this.tourism_invoice.service_type_id.name);
    this.checkingAccountFromControl.setValue(this.tourism_invoice.checking_account_id.name);
    let responsible_agent = this.tourism_invoice.responsible_agent_id.first_name && this.tourism_invoice.responsible_agent_id.last_name ?
        `${this.tourism_invoice.responsible_agent_id.first_name} ${this.tourism_invoice.responsible_agent_id.last_name}` : '';
    
        this.responsibleAgentFromControl.setValue(responsible_agent);
    let agent = this.tourism_invoice.agent_id.first_name && this.tourism_invoice.agent_id.last_name ?
        `${this.tourism_invoice.agent_id.first_name} ${this.tourism_invoice.agent_id.last_name}` : '';
    
        this.agentFromControl.setValue(agent);
    this.organizationFromControl.setValue(this.tourism_invoice.organization_id.group_name);
    let departure_station_name = this.tourism_invoice.detail_info.departure_station_id.name_ukr ? this.tourism_invoice.detail_info.departure_station_id.name_ukr : this.tourism_invoice.detail_info.departure_station_id.name_rus;
    this.departureStationFromControl.setValue(departure_station_name);
    let arrival_station_name = this.tourism_invoice.detail_info.arrival_station_id.name_ukr ? this.tourism_invoice.detail_info.arrival_station_id.name_ukr : this.tourism_invoice.detail_info.arrival_station_id.name_rus;
    this.arrivalStationFromControl.setValue(arrival_station_name);

    let individual_counterparty = this.tourism_invoice.detail_info.surname_id.last_name_native && this.tourism_invoice.detail_info.surname_id.first_name_native ?
    `${this.tourism_invoice.detail_info.surname_id.last_name_native} ${this.tourism_invoice.detail_info.surname_id.first_name_native}` : '';
  ...
  _get_total_amount_sum(){
    if(!this.tourism_invoice){
      return 0;
    }
    let supplier_cost = parseFloat(this.tourism_invoice.detail_info.supplier_cost.sum) || 0;
    let supplier_commision = parseFloat(this.tourism_invoice.detail_info.supplier_commision.sum) || 0;
    let forfeit = parseFloat(this.tourism_invoice.detail_info.forfeit.sum) || 0;
    let agency_services = parseFloat(this.tourism_invoice.detail_info.agency_services.sum) || 0;
    let other_services = parseFloat(this.tourism_invoice.detail_info.other_services.sum) || 0;

    let sum = supplier_cost + supplier_commision + forfeit + agency_services + other_services;
    this.tourism_invoice.detail_info.total_amount.sum = sum;
  ...
  _get_total_amount_mpe(){
    ...
    let supplier_cost = parseFloat(this.tourism_invoice.detail_info.supplier_cost.mpe) || 0;
    let supplier_commision = parseFloat(this.tourism_invoice.detail_info.supplier_commision.mpe) || 0;
    let forfeit = parseFloat(this.tourism_invoice.detail_info.forfeit.mpe) || 0;
    let agency_services = parseFloat(this.tourism_invoice.detail_info.agency_services.mpe) || 0;
    let other_services = parseFloat(this.tourism_invoice.detail_info.other_services.mpe) || 0;

    let mpe = supplier_cost + supplier_commision + forfeit + agency_services + other_services;
    this.tourism_invoice.detail_info.total_amount.mpe = mpe;
    return mpe.toFixed(3);
  }

  save_ticket(ticket: TourismInvoiceDetail){
    console.log('Save ticket');
    this.TourismService.create_tourism_invoice(ticket)
      .then((response: HttpResponse) => {
        this.toastr.successToastr('Залізничний квиток успішно збереженно.', 'Успішно!');
        //this.tourism_invoice = data;
      })
      .catch((err) => {
        console.log(err);
  ...
  update_ticket(ticket){
    ...
    this.TourismService.update_tourism_invoice(ticket)
      .then((response: HttpResponse) => {
  ...
  save_update(ticket: TourismInvoiceDetail){
    if(!ticket){
      // error occured
    }
    if(ticket._id){
  ...
  show_data(){
    console.log('==== Date: ', this.tourism_invoice.service_date)
  }
