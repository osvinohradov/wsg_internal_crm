...
import { TrainInvoiceInfo } from "../../models/tourism_invoice_info";
import { TourismService } from "../../services";
import { GroupInvoiceNameModel } from "../../../GroupInvoice/models";
import { TourismInvoiceDialogComponent } from "../invoice_popup/invoice_popup.component";
...
export class TourismInvoiceComponent implements OnInit {
  ...
  constructor(private TourismService: TourismService, private dialog: MatDialog){
  ...
  load_invoices_data(){
    ...
    this.TourismService.get_tourism_invoices(0, 0).subscribe((response: HttpResponse) => {
  ...
  edit_invoice(invoice_id) {
    this.is_loader_displayed = true;
    this.TourismService.get_tourism_invoice_by_id(invoice_id).subscribe((response: HttpResponse) => {
   ...
   * 
   */
  open_dialog(data): MatDialogRef<TourismInvoiceDialogComponent>{
    return this.dialog.open(TourismInvoiceDialogComponent, {
